#!/usr/bin/env python3
"""
Fine-tune Stable Diffusion for inpainting on your synthetic pairs.
"""
...
class InpaintingDataset(Dataset):
    """Dataset for inpainting: masked image + mask -> complete image."""
    
    def __init__(self, input_dir: Path, mask_dir: Path, gt_dir: Path, image_size=512, max_samples=None):
...
def train_inpainting(
    ...
    base_model: str = "runwayml/stable-diffusion-inpainting"  # Base model to fine-tune from (specialized inpainting model)
):
    ...
    if resume_from:
        ...
        pipeline = StableDiffusionInpaintPipeline.from_pretrained(resume_from)
    else:
    ...
    train_dataset_temp = InpaintingDataset(train_input_dir, train_mask_dir, train_gt_dir, image_size=image_size)
    num_train_steps = max(
        1,
        ...
        try:
            with torch.no_grad():
                for i, idx in enumerate(sample_indices):
                    ...
                    mask_np = (mask_vis.permute(1, 2, 0).numpy() * 255).astype(np.uint8)
                    # Resize input and mask to match if needed
                    if input_np.shape[:2] != gt_np.shape[:2]:
                        input_np = cv2.resize(input_np, (gt_np.shape[1], gt_np.shape[0]))
                    if mask_np.shape[:2] != gt_np.shape[:2]:
                        mask_np = cv2.resize(mask_np, (gt_np.shape[1], gt_np.shape[0]))
                    # Convert mask to 3 channels (RGB) to match other images for concatenation
                    if len(mask_np.shape) == 2 or mask_np.shape[2] == 1:
                        # Single channel mask - convert to 3 channels
                        mask_np = np.repeat(mask_np, 3, axis=2) if len(mask_np.shape) == 3 else np.repeat(mask_np[:, :, np.newaxis], 3, axis=2)
                    comparison = np.hstack([input_np, mask_np, result_np, gt_np])
    ...
    for epoch in range(num_epochs):
        ...
        for batch in progress_bar:
            with accelerator.accumulate(unet):
                ...
                # Condition on the masked input + mask to learn inpainting
                ...
                # This simulates the inpainting process: we'll learn to remove this noise
                noise = torch.randn_like(gt_latents)
                noisy_gt_latents = noise_scheduler.add_noise(gt_latents, noise, timesteps)
                
                # For inpainting: condition on the masked input latents
                # Blend masked input with noisy GT for conditioning
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fine-tune Stable Diffusion for inpainting")
    parser.add_argument("--train_input", type=str, default="data/pairs/inpaint/train/input",
                       help="Training input directory (masked images)")
    parser.add_argument("--train_mask", type=str, default="data/pairs/inpaint/train/mask",
                       help="Training mask directory")
    parser.add_argument("--train_gt", type=str, default="data/pairs/inpaint/train/gt",
                       help="Training ground truth directory")
    parser.add_argument("--val_input", type=str, default="data/pairs/inpaint/val/input",
                       help="Validation input directory")
    parser.add_argument("--val_mask", type=str, default="data/pairs/inpaint/val/mask",
                       help="Validation mask directory")
    parser.add_argument("--val_gt", type=str, default="data/pairs/inpaint/val/gt",
                       help="Validation ground truth directory")
    parser.add_argument("--output_dir", type=str, default="outputs/models/inpainting",
